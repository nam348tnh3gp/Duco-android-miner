...
#if defined(__APPLE__)
#define DUCO_EXPORT __attribute__((visibility("default")))
#else
#define DUCO_EXPORT
#endif
...
DUCO_EXPORT void get_logs(char *buffer, int buffer_size) {
    pthread_mutex_lock(&g_log_mutex);
    buffer[0] = '\0';
    int idx = g_log_head;
...
DUCO_EXPORT void get_new_logs(char *buffer, int buffer_size) {
    pthread_mutex_lock(&g_log_mutex);
    buffer[0] = '\0';
...
// ==================== HELPER FUNCTIONS ====================
static void duco_sleep_us(unsigned long microseconds) {
    struct timespec req;
...
static void log_startup_info(const char *username, const char *difficulty, const char *rig, int thread_count) {
    ...
    snprintf(buffer, sizeof(buffer), "%s" COLOR_YELLOW BLOCK_SYMBOL COLOR_RESET 
             "https://github.com/nam348tnh3gp/Duco-android-miner", COLOR_YELLOW);
    add_log(buffer);
    snprintf(buffer, sizeof(buffer), "%s" COLOR_YELLOW BLOCK_SYMBOL COLOR_RESET 
        ...
        while (g_running) {
            ...
            for (long long nonce = 0; nonce <= max_nonce && g_running; nonce++) {
                ...
                if (eff > 0 && (nonce % 5000 == 0)) {
                    if (!g_running) break;
                    duco_sleep_us((unsigned long)(eff / 100 * 1000000));
                }
            }
...
DUCO_EXPORT void start_mining(const char *username,
                  const char *key,
                  const char *diff,
                  const char *rig,
                  int threads,
    ...
    g_running = 1;
#if defined(__ANDROID__)
    // Android: keep the original nice support.
    setpriority(PRIO_PROCESS, 0, nice);
#else
    // iOS does not allow an app to arbitrarily change its process nice value.
    (void)nice;
#endif
...
DUCO_EXPORT int is_mining_running() {
    return g_running;
}